...
#folder paths
indir=${basedir}/In/enh
indir2=${basedir}/In/cl
outdir=${basedir}/Out
refdir=${basedir}/Ref

gzip -d -q ${indir}/*.gz || true
gzip -d -q ${indir2}/*.gz || true
...
if [ ! -d "$indir2" ]; then
 echo "Error: Unable to find $indir2"
 exit 1
fi
...
cmd="$exenam $endian -b y -z n -f %s_%p_%f -o $outdir $indir"
echo "Running command:"
echo $cmd
$cmd

# Convert classic images.
cmd="$exenam $endian -b y -z n -f %s_%p_%f -o $outdir $indir2"
echo "Running command:"
echo $cmd
$cmd
